...
命令来自两个渠道：系统**内置命令**（`/compact`、`/goal`、`/persona`）和宿主应用通过 API 注册的**自定义命令**。
...
切换 AI 在当前会话中扮演的角色/人设。输入角色名称后，AI 会采用该角色的词汇、优先级和典型推理模式，直到会话结束或被新命令覆盖。
...
| 参数 | 必填 | 说明 |
|:----:|:----:|------|
| 角色名称 | 是 | 角色描述，如 `senior-backend-engineer`、`产品经理`、`security-expert`。输入 `default` 恢复默认行为 |

### 工作机制

`/persona` 是一个 **会话级** 命令——触发后在整个会话期间持续生效。后端通过两个阶段实现角色保持：

| 阶段 | 说明 |
|------|------|
| **触发提示** | 首次执行时，注入系统消息激活角色 |
| **保持提示** | 后续轮次注入提醒，确保 AI 保持角色 |

```mermaid
flowchart LR
    A["👤 /persona senior-backend-engineer"] --> B["📝 注入系统提示"]
    B --> C["🧠 AI 采用角色"]
    C --> D["🔄 后续轮次保持角色"]
    D --> E["🔁 新 /persona 命令覆盖"]
...
| 关键规则 | 说明 |
|---------|------|
| **会话级生效** | 角色仅在当前会话有效，关闭会话后失效 |
| **可覆盖** | 输入新的 `/persona` 命令即可切换角色 |
| **可恢复** | 使用 `/persona default` 恢复默认行为 |
...
`/goal` 采用**特殊前缀处理**模式：用户输入 `/goal <条件>` 后，前端将其作为普通消息发送，后端通过命令注册表（`GoalWorkflow`）检测前缀并注入系统提示，引导 AI 进入目标设定流程。

与 `/compact` 等前端直接处理的命令不同，`/goal` 不会立即执行操作——它触发的是一个**多轮对话工作流**：

| 对比 | `/compact` | `/goal` |
|------|-----------|---------|
| 处理方式 | 前端直接发送 RPC | 特殊前缀 → 系统提示注入 |
| 执行时机 | 即时执行 | AI 理解后引导多轮交互 |
| 用户确认 | 不需要 | 需要用户确认目标条件 |
...
- [LLM 内置工具](/docs/features/llm-tools/) — 了解 AI 在对话中可调用的内置工具
