...
1. **HTML attribute**：`<rtc-agent lang="en-US">` 设置的 `lang` 属性（最高优先级）
2. **localStorage**：键 `rtc-agent-locale` 中保存的用户选择
3. **浏览器语言**：`navigator.language`，若在支持列表中则采用
4. **默认值**：回退到源语言 `zh-CN`
...
仅持久化语言偏好到 `localStorage`，不触发语言切换。适用于在组件挂载前保存用户选择：
